...
import sys
sys.path.insert(0, "/scratch/halmazan/WCTE/Geometry")
from Geometry.Device import Device

# cache geometry so you don't reopen every event
_HALL = None
_WCD = None
_PMT_POS = {}  # (mpmt_id,pmt_id) -> (x,y,z)

def _load_wcte_geo(geo_file="/scratch/halmazan/WCTE/Geometry/examples/wcte_bldg157.geo", wcd_index=0):
    ...
    _PMT_POS = {}
    for j, mpmt in enumerate(_WCD.mpmts):
        for k, pmt in enumerate(mpmt.pmts):
            pl = pmt.get_placement("design")   # or "est" if you want as-built
            loc = pl["location"]               # expected x,y,z
            _PMT_POS[(j, k)] = (float(loc[0]), float(loc[1]), float(loc[2]))
    return _WCD
...
def run_multilateration_timecal(
    ...
    *,
    ...
    robust_loss="soft_l1",
    f_scale=2.0,
    geo_file="/scratch/halmazan/WCTE/Geometry/examples/wcte_bldg157.geo",
    placement="design",
    **kwargs
):
